...
void addValue(vector<long long int> & treeSum, vector<long long int> & newTree, int l, int r, int nL, int nR, long long int value, int curIndex)
{
    if(l == nL && r == nL)
    {
        treeSum[curIndex] = value*(r - l + 1);
        newTree[curIndex] = value;
        return ;
    }
    int t = (l + r)/2;
    if(newTree[curIndex] != 0)
    {
        newTree[2*curIndex] = newTree[curIndex];
        newTree[2*curIndex + 1] = newTree[curIndex];
        newTree[curIndex] = 0;
        treeSum[2*curIndex] = newTree[2*curIndex]*(t - l + 1);
        treeSum[2*curIndex + 1] = newTree[2*curIndex + 1]*(r - t);
    }
    if(nL > t)
